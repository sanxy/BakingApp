...
import com.sanxynet.bakingapp.fragment.DetailStepFragment;
import com.sanxynet.bakingapp.fragment.IngredientFragment;
import com.sanxynet.bakingapp.fragment.StepFragment;
import com.sanxynet.bakingapp.media.ExoPlayerManager;
...
public class StepActivity extends BaseActivity
    ...
    private void infoDbVideo(int idData) {
        ...
        if (isEmpty) {
            Uri uri = BakingAppContract.StepEntry.CONTENT_URI;
            ...
            String selection = BakingAppContract.StepEntry._ID + " = ?";
            ...
            try {
                if (mCursor != null) {
                    while (mCursor.moveToNext()) {
                        setRecipeId(mCursor.getInt(mCursor.getColumnIndex(BakingAppContract.StepEntry.COLUMN_NAME_RECIPES_ID)));
                        setNavigationId(mCursor.getInt(mCursor.getColumnIndex(BakingAppContract.StepEntry.COLUMN_NAME_ID)));
                        setVideoUri(mCursor.getString(mCursor.getColumnIndex(BakingAppContract.StepEntry.COLUMN_NAME_VIDEO_URL)));
                        setThumbnailURL(mCursor.getString(mCursor.getColumnIndex(BakingAppContract.StepEntry.COLUMN_NAME_THUMBNAIL_URL)));
                        setShortDescription(mCursor.getString(mCursor.getColumnIndex(BakingAppContract.StepEntry.COLUMN_NAME_SHORT_DESCRIPTION)));
                        setDescription(mCursor.getString(mCursor.getColumnIndex(BakingAppContract.StepEntry.COLUMN_NAME_DESCRIPTION)));
    ...
    public void setNavigationIdMax(int recipeId) {
        ...
        String selection = BakingAppContract.StepEntry.COLUMN_NAME_RECIPES_ID + " = ?";

        String[] argSelection = new String[]{String.valueOf(recipeId)};

        String strOrderBy = BakingAppContract.StepEntry._ID + " DESC ";
        ...
        try {
            if (mCursor != null) {
                mCursor.moveToNext();
                mNavigationIdMax = mCursor.getInt(mCursor.getColumnIndex(BakingAppContract.StepEntry.COLUMN_NAME_ID));
    ...
    private int getFirstRow(int recipeId) {
        ...
        String sortOrder = BakingAppContract.StepEntry._ID + " ASC LIMIT 1 ";
        mCursor = getContentResolver().query(uri, arrProjection, selection, argSelection, sortOrder);

        try {
            if (mCursor != null) {
                mCursor.moveToNext();
                row = mCursor.getInt(mCursor.getColumnIndex(BakingAppContract.StepEntry._ID));
            }
        } catch (Exception e) {
    ...
    private void addRemoveWidget(int recipeId, String recipeName) {
        ...
        if (widgetId == getRecipeId()) {
            recipeId = 0;
            recipeName = "";
            showSnackbar(getString(R.string.widget_removed));
        }
        else{
            showSnackbar(getString(R.string.widget_added));
        }
